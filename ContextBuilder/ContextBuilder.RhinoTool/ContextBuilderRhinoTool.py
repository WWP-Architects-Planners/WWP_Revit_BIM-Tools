...
def build_road_surface_from_curve(crv, width):
    ...
    base_curve = preprocess_road_curve(crv, min_segment_len=1.25)
    if base_curve is None:
        return []
    base_curve.Transform(rg.Transform.Translation(0, 0, z))

    # Preferred path: build one constant-width boundary from left/right offsets.
    road_patch = build_road_patch_from_offsets(base_curve, half_w, tol)
    if road_patch and is_road_patch_reasonable(base_curve, width, road_patch):
        return road_patch

    # Fallback path for problematic curves.
    out = []
...
def total_brep_area(breps):
    area = 0.0
    for b in breps:
        area += compute_brep_area(b)
    return area


def is_road_patch_reasonable(curve, width, breps):
    if not breps:
        return False
    expected = max(1.0, float(curve.GetLength()) * float(width))
    actual = total_brep_area(breps)
    # Too large means likely bow-tie / accidental long cap bridge.
    if actual > expected * 2.0:
        return False
    # Too tiny likely means offset failure.
    if actual < expected * 0.2:
        return False
    return True


def preprocess_road_curve(curve, min_segment_len=1.25):
    points = extract_curve_points(curve)
    if len(points) < 2:
        return None
    cleaned = [points[0]]
    for p in points[1:]:
        if cleaned[-1].DistanceTo(p) >= min_segment_len:
            cleaned.append(p)
    if len(cleaned) < 2:
        return None

    # Remove nearly collinear interior points to reduce tiny wedge artifacts.
    simplified = [cleaned[0]]
    angle_tol = math.radians(4.0)
    ...
    if len(simplified) == 2:
        return rg.LineCurve(simplified[0], simplified[1])
    poly = rg.Polyline(simplified)
    return poly.ToNurbsCurve() if poly.IsValid else curve.DuplicateCurve()


def extract_curve_points(curve):
    points = []
    segments = list(curve.DuplicateSegments() or [])
    if segments:
        for i, seg in enumerate(segments):
            ...
            if i == 0:
                points.append(a)
            if not points or points[-1].DistanceToSquared(b) > 1e-12:
                points.append(b)
        return points

    t_vals = curve.DivideByCount(24, True)
    if t_vals and len(t_vals) > 1:
        for t in t_vals:
...
def build_road_patch_from_offsets(center_curve, half_w, tol):
    plane = rg.Plane.WorldXY
    left_candidates = center_curve.Offset(plane, half_w, tol, rg.CurveOffsetCornerStyle.Sharp)
    right_candidates = center_curve.Offset(plane, -half_w, tol, rg.CurveOffsetCornerStyle.Sharp)
    if not left_candidates or not right_candidates:
        return []

    left = max(left_candidates, key=lambda c: c.GetLength())
    right = max(right_candidates, key=lambda c: c.GetLength())

    ls = left.PointAtStart
    le = left.PointAtEnd
    rs = right.PointAtStart
    re = right.PointAtEnd

    # Choose pairing that minimizes cap distance and avoids bow-tie boundaries.
    dist_same = ls.DistanceTo(rs) + le.DistanceTo(re)
    dist_cross = ls.DistanceTo(re) + le.DistanceTo(rs)
    if dist_same <= dist_cross:
        right_path = right.DuplicateCurve()
        right_path.Reverse()
        cap_end = rg.LineCurve(le, re)
        cap_start = rg.LineCurve(rs, ls)
    else:
        right_path = right.DuplicateCurve()
        cap_end = rg.LineCurve(le, rs)
        cap_start = rg.LineCurve(re, ls)

    joined = rg.Curve.JoinCurves([left, cap_end, right_path, cap_start], tol)
    if not joined:
        return []

    boundary = max(joined, key=lambda c: c.GetLength())
    if not boundary.IsClosed:
        return []
    self_events = rg.Intersect.Intersection.CurveSelf(boundary, tol)
    if self_events and len(self_events) > 0:
        return []
...
class ContextBuilderDialog(forms.Dialog[bool]):
    def __init__(self):
        ...
        self.chk_union_roads.Checked = False
            ...
            if "roads" in layers and road_curves_by_type:
                ...
                if road_breps_raw:
                    ...
                    if self.chk_union_roads.Checked and len(road_breps_raw) > 1:
                        raw_area = total_brep_area(road_breps_raw)
                        union = rg.Brep.CreateBooleanUnion(road_breps_raw, sc.doc.ModelAbsoluteTolerance)
                        if union and len(union) > 0:
                            union_list = list(union)
                            union_area = total_brep_area(union_list)
                            # Union should not increase area; reject suspicious bridge artifacts.
                            if union_area <= (raw_area * 1.05):
                                roads_to_add = union_list
                                self.append_log("Road union accepted: raw_area={:.2f}, union_area={:.2f}".format(raw_area, union_area))
                            else:
                                self.append_log("Road union rejected (area jump): raw_area={:.2f}, union_area={:.2f}".format(raw_area, union_area))
                        else:
